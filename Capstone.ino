...
void executeSecondCode(int shockLevel) {
  ...
  if (mySerial.available()) {
    String gpsData = mySerial.readStringUntil('\n'); // GPS 데이터 읽기
    if (gpsData.startsWith("$GPRMC")) { // 유효한 GPS 데이터인지 확인
      ...
      if (parseGPS(gpsData, latitude, longitude)) {
        ...
        // Firebase에 GPS 데이터 및 충격 단계 업로드
        uploadToFirebase(latitude, longitude, shockLevel, String(timeString));

        // SMS 보내기
        sendSMS(latitude, longitude, shockLevel, String(timeString));
...
bool parseGPS(String gpsData, float &latitude, float &longitude) {
  ...
  for (int i = 0; i < gpsData.length(); i++) {
    if (gpsData.charAt(i) == ',') {
      ...
      if (found == 3) {
        // 위도 추출
        temp = gpsData.substring(index + 1, i);
        float rawLatitude = temp.toFloat();
        int latDegrees = int(rawLatitude / 100);
        float latMinutes = rawLatitude - latDegrees * 100;
        latitude = latDegrees + (latMinutes / 60.0); // 위도를 도 단위로 변환
        index = i;
      }
      if (found == 5) {
        // 경도 추출
        temp = gpsData.substring(index + 1, i);
        float rawLongitude = temp.toFloat();
        int lonDegrees = int(rawLongitude / 100);
        float lonMinutes = rawLongitude - lonDegrees * 100;
        longitude = lonDegrees + (lonMinutes / 60.0); // 경도를 도 단위로 변환
        index = i;
        return true; // 파싱 완료
      }
      index = i;
...
void uploadToFirebase(float latitude, float longitude, int shockLevel, String timestamp) {
  ...
  // 하위 경로 생성
  String path = "/event_data/" + eventKey;
